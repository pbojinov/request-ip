...
import requestIp from '../src/index';

test.createStream().pipe(tapSpec()).pipe(process.stdout);

// Setup local server for testing
const serverInfo: { host: string } = {
  host: '127.0.0.1',
};
...
test('x-forwarded-for', t => {
  ...
  server.on('listening', () => {
    ...
    request(options, (error, response, found) => {
      if (!error && response.statusCode === 200) {
        // make sure response ip is the same as the one we passed in
        const lastIp = options.headers['x-forwarded-for'].split(',')[0].trim();
...
test('request-ip.mw - used with no arguments', t => {
  ...
  const mockReq: any = { headers: { 'x-forwarded-for': '111.222.111.222' } };
  mw(mockReq, null, () => {
